...
impl BrowserSecretBridge {
    ...
    pub fn request_connection(
        ...
        self.request_to_connection(connection, command, payload, timeout)
    }
...
fn handle_pipe_connection(mut pipe: std::fs::File, inner: Arc<SecretInner>) -> Result<(), String> {
    ...
    lock_unpoisoned(&inner.connections).insert(
        connection_id.clone(),
        SecretConnection {
            browser,
            sender: outbound_tx,
    ...
    let writer = std::thread::spawn(move || {
        while let Ok(message) = outbound_rx.recv() {
            if write_frame(&mut pipe, &message).is_err() {
                break;
            }
        }
    });
    ...
    let removed = remove_connection_generation(&inner.connections, &connection_id, generation);
    if removed {
        let mut events = lock_unpoisoned(&inner.events);
        while events.len() >= MAX_QUEUED_EVENTS {
            events.pop_front();
