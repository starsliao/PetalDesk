...
| MFA | `tools/mfa/vault.json` | 整体加密保险库 | 账户量小，单个 AEAD 密文便于完整认证；DPAPI 与恢复密码双包装支持本机免密和跨机迁移 |

SQLite FTS 只保存可重建的搜索索引，位于 `%LOCALAPPDATA%/PetalDesk`，不属于需要迁移的权威数据。
...
2. 权威文件先写同目录临时文件并刷新到磁盘，再通过 Windows 原子替换发布。
...
首次使用 MFA 时必须先设置恢复密码，完成后才能写入账户。保险库的随机数据密钥同时保留 Windows DPAPI 包装和 Argon2id 恢复密码包装；恢复密码只参与密钥解包，不直接加密账户正文，也不会写入磁盘。复制完整数据目录后，在新电脑首次打开 MFA 时输入一次恢复密码，飞花会验证保险库并为当前 Windows 用户重新生成 DPAPI 包装，后续继续免密打开。每份 MFA 备份都包含恢复密码包装，因此可随主保险库一起迁移和恢复。
