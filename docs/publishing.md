...
Windows 10/11 x64 与 macOS 12+ 共享便签、甘特图、计时器、提醒、系统通知、MFA 和普通截图能力。`0.6.0` 在 Windows 新增本地密码保险库，并把经确认的自动填充和登录信息检测整合进 Firefox 长截图扩展；`0.6.1` 修复密码管理器首次启用时对 MFA 全局恢复密码的复用与修改体验。Chrome/Edge 的密码能力延期。Windows 继续提供长截图、自动滚动和 Native Messaging Host。macOS 使用 Keychain 提供 MFA 本机免密解锁，并支持普通区域截图，但当前不提供密码管理器、长截图、浏览器联动、Native Messaging 注册或自动更新。
...
- `PetalDesk_0.6.1_x64-setup.exe`
- `PetalDesk_0.6.1_x64-setup.exe.sig`
- `latest.json`
- `PetalDesk_0.6.1_universal.dmg`
- `PetalDesk_Firefox_AMO-upload_0.6.1.zip`
- `PetalDesk_Firefox_AMO-source_0.6.1.zip`

Windows `0.5.2` 及后续版本可以通过客户端自动更新到 `0.6.1`，无需先在本机手工打包或安装。macOS 的 `0.5.0` 是首个公开版本，`0.6.1` 可以直接覆盖升级。MFA 保险库继续兼容已有 DPAPI/Keychain 包装；Windows 密码保险库使用独立数据密钥，并与 MFA 协调同一个全局恢复密码。
...
首次 AMO 发布时应先把包含这两个页面的 `main` 分支推送并等待 Pages 部署成功，再创建 `v0.6.1` 标签和提交 AMO，避免审核材料引用尚未上线的页面。
...
<https://github.com/starsliao/PetalDesk/releases/tag/v0.6.1>
