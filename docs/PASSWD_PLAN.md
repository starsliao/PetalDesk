...
- 当前 AMO 公开提交版本号为 `0.6.3`（桌面端 `0.6.0` 已先行发布）。`package.json`、Cargo、Tauri 和两个浏览器 manifest 必须在提交标签前保持一致；Firefox AMO 公开提交也使用此版本号。
...
- `0.6.3` 公开上架继续使用 AMO Developer Hub 手工上传。标签 CI 只生成 AMO 上传 ZIP 和审核源码 ZIP，不读取 AMO API secret，也不在桌面 Release 成功前占用不可重复的扩展版本号。
...
- 日常由 Windows DPAPI 自动解锁；窗口关闭或显式锁定时清理解密状态、待处理请求和剪贴板，不再按闲置时间自动锁定。短时显示的密码与敏感剪贴板仍按各自时限自动清理。
