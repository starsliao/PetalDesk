...
- 当前 AMO 公开提交版本号为 `0.6.2`（桌面端 `0.6.0` 已先行发布）。`package.json`、Cargo、Tauri 和两个浏览器 manifest 必须在提交标签前保持一致；Firefox AMO 公开提交也使用此版本号。
...
- `0.6.2` 公开上架继续使用 AMO Developer Hub 手工上传。标签 CI 只生成 AMO 上传 ZIP 和审核源码 ZIP，不读取 AMO API secret，也不在桌面 Release 成功前占用不可重复的扩展版本号。
