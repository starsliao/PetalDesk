...
[产品网站](https://starsliao.github.io/PetalDesk/) · [GitHub](https://github.com/starsliao/PetalDesk) · [下载 Windows x64](https://github.com/starsliao/PetalDesk/releases/download/v0.8.0/PetalDesk_0.8.0_x64-setup.exe) · [下载 macOS Universal](https://github.com/starsliao/PetalDesk/releases/download/v0.8.0/PetalDesk_0.8.0_universal.dmg)
...
`0.8.0` 把密码管理器与本地 TOTP 验证器联动起来：一个密码账户可关联一个 MFA 账户；飞花填入账号密码，或用户手动输入与唯一已保存账户完全一致时，会启动最长 5 分钟的二次验证 journey。唯一可信的 6/7/8 位 TOTP 字段可自动填入，模糊或多个字段会先显示“一键填充”，跨 origin 页面只接受首次明确确认并记住的精确 HTTPS origin；飞花始终不点击登录按钮、不发送 Enter，也不主动提交表单。Firefox 弹窗账户列表改为固定卡片，直接提供复制用户名、复制密码、复制 MFA 和右上角二次确认删除；MFA 验证码由桌面端直接写入系统剪贴板，并在失效时自动清除。关于页继续在当前版本旁显示正式安装包的真实打包时间。

`0.7.4` 修复 Firefox 140+ 在 163 邮箱跨源登录 iframe 中无法填充账号密码、点击登录后不提示保存或更新的问题；扩展改用 Firefox 提供的标签页、frame 与 document 身份绑定请求，覆盖 163 的语义登录按钮，并把 iframe 身份贯穿到填充结果。跨源委派仍为默认拒绝，仅显式允许 `mail.163.com` 到 `dl.reg.163.com` 的单向登录委派；共享托管域的跨租户 iframe 继续拒绝。关于页现在会在当前版本旁显示本次安装包的打包时间。`0.7.3` 按使用反馈简化密码体验：扩展弹窗中点击账户直接填充（不再要求页面二次确认，填充后页面短暂提示且绝不自动提交）；登录检测改为始终开启，提交登录即提示保存/更新（不再判断登录是否成功），并移除密码管理器中的检测开关与首次引导；弹窗账户新增菜单，可复制用户名、复制密码（由桌面端写入剪贴板，凭据不经过扩展）和删除账户（二次确认）；Windows 安装器检测到旧版本时直接覆盖更新，不再询问卸载。`0.7.2` 修复了密码通道自 0.6.3 以来的根因：Windows 同步命名管道在挂起读时另一方向写入会永久阻塞，桌面端与 Native Host 双端均改为重叠 I/O，连接不再每几秒超时重连；同时修复扩展在通道恢复后仍上报过期站点 origin 的问题（标签页激活时一律读取实时 URL），并补充角标/填充全链路的连接诊断事件。`0.7.1` 将远程桌面与录屏的敏感窗口保护改为可选设置：默认在远程桌面会话中也允许打开 MFA 验证器与密码管理器，窗口内容不再对截图和录屏隐藏；需要旧行为时可在 设置 > 隐私与安全 中开启"保护敏感窗口"。`0.7.0` 为 Firefox 扩展新增站点账户数角标与诊断/账户弹窗，弹窗中点击账户即可在当前页发起填充；登录检测现在支持"保存为新账户或选择更新任一既有账户"，保险库支持浏览器后台会话，密码窗口关闭时角标、捕获与填充仍可用，手动锁定后立即停止。Firefox"与扩展开发者分享身份验证信息"改为安装时必要权限，不再要求工具栏二次授权。`0.6.3` 修复 Firefox 原生消息通道断开后造成的飞花卡顿、密码权限误报和扩展按钮无反馈，并为通道握手与重连增加超时保护。`0.6.2` 已修复密码管理器图标提示、窗口关闭和手动锁定交互，取消闲置 15 分钟自动锁定；远程桌面会话中不再打开会被系统隐藏的 MFA 与密码管理器窗口，而是直接说明原因并提示回到本机使用。

| 能力 | Windows 10/11 x64 | macOS 12+（Intel / Apple Silicon） |
| --- | --- | --- |
| 便签、甘特图、计时器、提醒 | 支持 | 支持 |
| MFA 本机免密保护 | Windows DPAPI | macOS Keychain |
| 密码管理器 | 支持；Firefox 扩展提供账户角标、卡片式快捷复制、点击账户直接填充、登录信息检测与关联 TOTP 填充 | 当前版本暂不支持 |
...
- 密码管理器：使用独立的 XChaCha20-Poly1305 保险库保存多个站点账户，共享 MFA 恢复密码并由 Windows DPAPI 日常解锁。Firefox 扩展角标会显示当前站点已保存的账户数量，点击图标可查看连接诊断和卡片式账户列表；账户可直接填充，或分别复制用户名、密码与已关联的 MFA 验证码。登录信息检测始终开启，用户提交登录后立即提示保存新账户；用户名或密码与已存账户不一致时，可选择保存为新账户或更新任一既有账户。密码账户可关联本地 TOTP；在受信任的登录 journey 内，扩展只填入唯一可信字段，模糊字段需用户确认，并且绝不主动提交。密码窗口关闭时这些能力仍可用，在密码管理器或 MFA 中手动锁定后立即停止。秘密只在当前用户的本机内存通道中按需处理，不写入浏览器存储、诊断日志或截图文件队列。
...
Windows 上的 `0.5.2` 是自动更新桥接版本；已安装该版本或更高版本的用户可直接在飞花内更新到 `0.8.0`，无需重新运行本地安装流程。macOS 的 `0.5.0` 是首个公开版本，`0.8.0` 可以直接覆盖升级，但当前仍使用手动下载安装。首次启动时会自动识别旧的 `飞花/.feihua` 存储布局、旧便签元数据和旧甘特图数组格式，转换到当前 `.petaldesk/` 结构；便签正文 `note.md` 不会被改写，甘特图转换前会保留迁移备份。密码保险库 v1 会透明读取；首次成功写入 v2 前保留已认证备份，原有加密 AAD 不变。同机升级不需要输入全局恢复密码；恢复密码由 MFA 与密码管理器全局共用，用于跨设备恢复两个保险库，以及主动导出单个 MFA 账户。旧数据损坏或格式版本过新时，飞花会保留原文件并阻止静默覆盖。

## 安装与运行

Windows 新用户可下载 [Windows x64 安装包](https://github.com/starsliao/PetalDesk/releases/download/v0.8.0/PetalDesk_0.8.0_x64-setup.exe) 后按向导操作。安装器会检查 WebView2；缺少时从微软官方地址显示进度并下载、静默安装，然后继续安装飞花 - PetalDesk。联网安装包因此更小。`0.5.2` 起默认开启自动检查与后台下载；“关于飞花 - PetalDesk”中可以查看当前版本及安装包打包时间、手动检查或关闭自动更新，下载完成后仍由用户选择何时重启安装。

没有 WebView2 或下载权限时，安装器会明确提示失败原因，不会静默留下无法启动的程序。未签名构建可能显示“未知发布者”，这是 Windows 对代码签名的正常提示。

macOS 用户下载 [Universal DMG](https://github.com/starsliao/PetalDesk/releases/download/v0.8.0/PetalDesk_0.8.0_universal.dmg)，将应用拖入“应用程序”即可。同一个 DMG 同时包含 Intel（x86_64）和 Apple Silicon（arm64）代码，不需要按芯片下载不同安装包。普通截图首次使用时，请在“系统设置 > 隐私与安全性 > 屏幕录制”中允许 PetalDesk；未签名、未公证的测试构建可能被 Gatekeeper 阻止，正式发布方式见 [`docs/publishing.md`](docs/publishing.md)。
...
- Firefox 扩展：在 Windows 上提供长截图协作，以及站点账户数角标、连接诊断与账户弹窗、用户点选账户后触发的密码填充和登录信息检测；Chrome、Edge 首版继续只用于长截图。
